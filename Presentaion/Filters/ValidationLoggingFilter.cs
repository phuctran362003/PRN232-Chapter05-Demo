...
public class ValidationLoggingFilter : IActionFilter
{
    public void OnActionExecuting(ActionExecutingContext context)
    {
        ...
        if (!context.ModelState.IsValid)
        {
            ...
            foreach (var error in errors) Console.WriteLine($"   - Property '{error.Property}': {error.Error}");
        }
        else
        {
            Console.WriteLine(
                $"[VALIDATION] Model state is valid for action '{context.ActionDescriptor.DisplayName}'");
        }
    }
    ...
    {
        Console.WriteLine(
            $"[VALIDATION] Action '{context.ActionDescriptor.DisplayName}' execution completed with status code: {context.HttpContext.Response.StatusCode}");
    }
}